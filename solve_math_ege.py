...
def solve_math_ege():
    ...
    print(f"📈 Начинаем решать Математику! Всего задач: {total}, уже решено: {solved}")
    ...
    for i, task in enumerate(tasks):
        ...
        prompt = f"""Ты — эксперт ЕГЭ по профильной математике. Реши задание.
Текст задания:
\"\"\"
{task.get('task_text', '')}
\"\"\"
ВЕРНИ ТОЛЬКО ОТВЕТ! Никаких слов "Ответ", решений или единиц измерения.
Если ответ — целое число, пиши только его (например, -5 или 12).
Если ответ — десятичная дробь, используй ЗАПЯТУЮ (например, 0,5).
Никаких пробелов."""

        try:
            input_data = {"prompt": prompt}
            
            # Ищем картинку на сервере
            ...
            if img_rel_path and os.path.exists(img_rel_path):
                ...
                print(f"   🖼️ Прикреплена картинка: {img_rel_path}")
    ...
    print(f"🎉 УРА! Математика полностью решена!")
