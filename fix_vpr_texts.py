...
MARK = re.compile(r'(по\s+тексту|из\s+текста|прочт[а-я]+\s+текст|текста\s+и\s+выполните|к\s+тексту|над\s+текстом|в\s+тексте|отрывок|стихотворени)', re.I)
...
for jf in sorted(os.listdir("questions")):
    ...
    for t in tasks:
        if not MARK.search(t["task_text"]): continue
    ...
    print(f"💾 {jf}")
print(f"\n✅ Текстов приклеено: {fixed} | не найдено PDF: {missed}")
