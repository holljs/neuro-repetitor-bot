...
def smart_crop_and_stitch(topic, p1, p2=None):
    ...
    # НОВЫЙ "ЖАДНЫЙ" ПРОМПТ
    prompt = (
        f"Ты — эксперт по оцифровке учебников. Найди все задачи на стр {p1}. "
        "КРИТИЧЕСКИЕ ПРАВИЛА:\n"
        "1. Каждое задание (а, б, в) — это отдельный объект в JSON.\n"
        "2. Если у группы задач (например, №78) есть общий заголовок 'Решите уравнение' или 'Упростите выражение', "
        "ты ОБЯЗАН включить этот текст в координаты (box_2d) для КАЖДОГО подпункта.\n"
        "3. Если задание является пунктом 'б', захватывай область так, чтобы было видно общее условие задачи.\n"
        "4. Координаты должны охватывать: [Общее условие] + [Номер] + [Само задание].\n\n"
        "Верни строго JSON список объектов: "
        "{'number': 'номер_с_буквой', 'box_2d': [ymin, xmin, ymax, xmax], 'needs_stitch': false}"
    )
