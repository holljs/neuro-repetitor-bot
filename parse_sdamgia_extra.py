...
JUNK_RE = re.compile(r'(Решения заданий с развернутым ответом[^.]*\.|Запишите решение на бумаге\.|Развернуть|Свернуть|Воспользуйтесь плеером[^.]*\.)', re.I)
...
def clean_text(s):
    return re.sub(r"\s+", " ", JUNK_RE.sub(" ", s.replace("\u00ad","").replace("\xa0"," "))).strip()
...
def save_image(src, base):
    ...
    try:
        ...
        p = f"{IMG_DIR}/sd_{key}{ext}"
        open(p,"wb").write(ir.content); return p
    except: return None
...
def check_id(base, tid):
    try:
        ...
        if r.status_code == 200 and "prob_maindiv" in r.text:
            r.encoding = "utf-8"; return tid, r.text
    except: pass
    return None, None
...
def parse_test(base, html, seen):
    ...
    for main in soup.find_all("div", class_="prob_maindiv"):
        ...
        imgs = own_imgs
        if not imgs and re.search(r'(фото|рисунк|изображ|знак)', txt, re.I) and last_imgs and cur_type and cur_type == last_type:
            imgs = last_imgs
        if own_imgs:
            last_imgs, last_type = own_imgs, cur_type
        seen.add(did)
...
for sub, subj, grade, start_id in SUBS:
    ...
    for t in all_tasks:
        ...
        if h not in hashes: hashes.add(h); uniq.append(t)
    if uniq:
        # Уникальные ID
        for t in uniq:
            num = t['id'].split('_')[-1]
            t['id'] = f"vpr_{subj}_{grade}_sd_{num}"
